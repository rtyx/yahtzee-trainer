...
test('full house can score nominal dice total', () => {
  assert.equal(scoreCategory([0, 2, 0, 3, 0, 0], 8, { combinationScore: 'sum' }), 16);
});
...
test('combination categories use nominal dice values in sum mode', () => {
  ...
  assert.equal(scoreCategory([0, 0, 0, 0, 5, 0], 11, { combinationScore: 'sum' }), 25);
});
